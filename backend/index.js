...
async function batchWriteWithRetries(dynamo, requestItems, maxRetries = 5) {
  ...
  if (Object.keys(unprocessed).length > 0) {
    throw new Error('BatchWrite did not complete after retries; some items remain unprocessed.');
  }
...
function buildItemsForDataset(rows, headerOriginalOrder, category) {
  ...
  const pkNormalized = String(DYNAMO_PK || 'id').trim().toLowerCase();
  ...
  if (!hasTotalAlready) { headerNormalizedOrder.push(totalNorm); sanitizedHeaders.push(totalOrig); }

  const excludedForSum = new Set([pkNormalized, 'nombre', 'fotos', 'pierres', 'piedras', 'pictures', 'photos']);
  const sourceHeaders = sanitizedHeaders.filter((h) => String(h).trim().toLowerCase() !== totalNorm);
  ...
  const filtered = normalized.filter((r) => {
    const nomVal = (r.nom !== undefined && r.nom !== null) ? String(r.nom).trim().toLowerCase() : '';
    const nombreVal = (r.nombre !== undefined && r.nombre !== null) ? String(r.nombre).trim().toLowerCase() : '';
  ...
  for (let idx = 0; idx < filtered.length; idx++) {
    ...
    const pierresRaw = (r.pierres !== undefined && r.pierres !== null && String(r.pierres).trim() !== '') ? r.pierres : r.piedras;
    const pierresStr = String(pierresRaw ?? '').trim();
    if (pierresStr === '') { missing.push({ index: idx, row: r }); continue; }

    let idVal = null;
    if (pkIndex !== -1) {
      const pkKey = headerNormalizedOrder[pkIndex];
      const candidate = r[pkKey];
      if (candidate !== undefined && candidate !== null && String(candidate).trim() !== '') idVal = String(candidate).trim();
    }
    if (!idVal) idVal = uuidv4();

    if (seenIds.has(idVal)) throw new Error(`Duplicate ID "${idVal}" detected in import for category "${category}" (row index ${idx}).`);
    seenIds.add(idVal);
    ...
    for (const hdr of Object.keys(r)) {
      ...
      else warnings.push({ row: idx, column: hdr, value: raw, message: 'Non-numeric value treated as 0 for Total' });
    }
...
app.post('/parse', upload.single('file'), async (req, res) => {
  try {
    ...
    const providedCategory = (req.query.category || (req.body && req.body.category)) || null;
    const replaceMode = (req.query.replace === 'true' || req.query.replace === '1' || req.query.replace === 'yes');
    const updateMode = (req.query.update === 'true' || req.query.update === '1' || req.query.update === 'yes');
    const dryRun = (req.query.dry === 'true' || req.query.dry === '1' || req.query.dry === 'yes');

    if (req.file && req.file.buffer) buffer = req.file.buffer;
    else if (req.body && typeof req.body === 'string' && req.body.trim().length > 0) text = req.body;
    else return res.status(400).json({ error: 'No CSV or spreadsheet provided.' });
    ...
    if (buffer) {
      ...
      if (isZip) {
        ...
        if (!sheetNames || sheetNames.length === 0) return res.status(400).json({ error: 'XLSX contains no sheets' });
        for (const sheetName of sheetNames) {
          const sheet = workbook.Sheets[sheetName];
          const parsedRecords = XLSX.utils.sheet_to_json(sheet, { defval: null });
          if (!Array.isArray(parsedRecords) || parsedRecords.length === 0) continue;
          const rawHeaders = Object.keys(parsedRecords[0]).map((h) => String(h || '').trim());
          const headerOriginalOrder = sanitizeHeaderList(rawHeaders);
          const cleanedRecords = parsedRecords.map((r) => {
            const cleanRow = {};
            headerOriginalOrder.forEach((h) => { cleanRow[h] = r[h] === undefined ? null : r[h]; });
            return cleanRow;
          });
          const category = (categoriesMapping[sheetName] || providedCategory || sheetName);
          datasets.push({ sheetName, category, parsedRecords: cleanedRecords, headerOriginalOrder });
        }
        parseType = 'xlsx';
      } else {
    ...
    if (!datasets.length && text != null) {
      ...
      const rawHeaders = Object.keys(records[0]).map((h) => String(h || '').trim());
      const headerOriginalOrder = sanitizeHeaderList(rawHeaders);
      ...
      const category = providedCategory || (req.body && req.body.categories ? JSON.parse(req.body.categories)['dataset'] : null);
      datasets.push({ sheetName: 'dataset', category, parsedRecords: cleanedRecords, headerOriginalOrder });
      parseType = 'csv';
    }

    if (datasets.length === 0) return res.status(400).json({ error: 'No datasets found in the uploaded file' });

    const allWork = [];
    for (const ds of datasets) {
      try {
        const built = buildItemsForDataset(ds.parsedRecords, ds.headerOriginalOrder, ds.category);
        allWork.push({ sheetName: ds.sheetName, category: ds.category, ...built });
      } catch (err) {
        return res.status(400).json({ error: `Failed to build items for sheet ${ds.sheetName}: ${err.message}` });
      }
    }

    const bad = allWork.find((w) => w.missing && w.missing.length > 0);
    if (bad) {
      return res.status(400).json({ error: 'Some rows are missing Pierres/Piedras in category: ' + bad.category, examples: bad.missing.slice(0, 5) });
    }
    ...
    for (const work of allWork) {
      ...
      if (replaceMode) {
        let deleteResult;
        try { deleteResult = await deleteAllItems(dynamo, work.category); } catch (err) { console.error('Error deleting items for category', work.category, err); return res.status(500).json({ error: `Failed to delete items for category ${work.category}`, details: String(err) }); }
        const itemsToWrite = [...work.items, schemaItem];
        let putResult;
        try { putResult = await putItems(dynamo, itemsToWrite); } catch (err) { console.error('Error writing items for category', work.category, err); return res.status(500).json({ error: `Failed to write items for category ${work.category}`, details: String(err) }); }
        results.push({ sheetName: work.sheetName, category: work.category, mode: 'replace', deletedCount: deleteResult.deletedCount, writtenCount: putResult.writtenCount, warnings: work.warnings || [], defaultSelectedColumn: 'Total' });
      } else {
        const itemsToWrite = [...work.items, schemaItem];
        let putResult;
        try { putResult = await putItems(dynamo, itemsToWrite); } catch (err) { console.error('Error upserting items for category', work.category, err); return res.status(500).json({ error: `Failed to upsert items for category ${work.category}`, details: String(err) }); }
        results.push({ sheetName: work.sheetName, category: work.category, mode: 'update', writtenCount: putResult.writtenCount, warnings: work.warnings || [], defaultSelectedColumn: 'Total' });
      }
    }
...
app.get('/columns', async (_req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to aggregate columns', details: String(err) });
  }
});
...
app.delete('/category/:name/column/:columnName', async (req, res) => {
  ...
  try {
    ...
    if (schemaItem) {
      ...
      const filteredOrig = []; const filteredNorm = [];
    ...
    return res.status(500).json({ error: 'Failed to delete column', details: String(err) });
  }
});
